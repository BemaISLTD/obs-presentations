// Scene 17 owns these design primitives so it can be edited independently.
const ICONS = {
  people:
  ...
  trophy:
    '<svg viewBox="0 0 24 24"><path d="M8 4h8v5a4 4 0 0 1-8 0V4ZM8 6H4v2c0 3 2 4 5 4M16 6h4v2c0 3-2 4-5 4M12 13v5M8 21h8M9 18h6"/></svg>',
  target:
    '<svg viewBox="0 0 24 24"><circle cx="11" cy="13" r="8"/><circle cx="11" cy="13" r="3"/><path d="m13 11 7-7M16 4h4v4"/></svg>',
};
...
function renderBrand({ enrollment = false } = {}) {
  return `
    <div class="broadcast-brand absolute left-[48px] top-[34px] z-20 flex items-center gap-3 border-0 bg-transparent font-sans text-bema-navy shadow-none" style="background:transparent!important;border:0!important;box-shadow:none!important;backdrop-filter:none!important">
      <img class="h-[72px] w-auto bg-transparent" style="background:transparent!important" src="/assets/logos/bemahub-reference-mark.svg" alt="" />
      <span class="brand-wordmark bg-transparent" style="background:transparent!important;border:0!important;box-shadow:none!important" data-asset-wrapper>
        <img class="broadcast-wordmark h-[74px] w-auto bg-transparent" style="background:transparent!important" src="/assets/logos/bemahub-wordmark.svg" alt="BemaHub" data-asset-image />
...
const config = {
  title: "CAMPAIGNS PAGE WALKTHROUGH",
  subtitle: "Discover creator-led campaigns.",
  layout: "dashboard",
  items: [
    card(
      "Sound of Tomorrow",
      "by NovaWave · Participation Level",
      "music",
      "FEATURED",
      "blue",
      "campaign-featured",
    ...
    card(
      "Loop Activity",
      "218 Builders joined Stage to Spotlight",
      "signal",
      "REAL TIME",
      "blue",
      "campaign-activity",
    ...
    card(
      "Explore Campaigns",
      "Find a campaign that moves you.",
      "heart",
      "START",
      "cyan",
      "campaign-cta",
...
export const scene17 = {
  presenterZone: "left",
  renderUnderlay() {
    const body = `<div class="scene17-campaign-browser h-full"><section class="scene17-campaigns rounded-panel border border-sky-200/70 bg-white/95 shadow-card"><header>${icon("star")}<strong>Featured Campaigns</strong><nav><b>All</b><span>Trending</span><span>New</span><span>Ending Soon</span></nav><label>⌕　Search campaigns...</label></header><div class="scene17-card-grid">${campaigns.map(([title, creator, badge, art]) => `<article class="overflow-hidden rounded-2xl border border-sky-100 bg-white shadow-sm"><div class="scene17-art art-${art}">${badge ? `<b>${badge}</b>` : ""}</div><h3 class="font-bold text-xl">${title}</h3><p>by ${creator}</p><footer><span>Participation Level</span><i>${icon("people")}${icon("people")}${icon("people")}${icon("people")}</i></footer></article>`).join("")}</div><a class="scene17-view-all">View all campaigns　→</a></section><aside class="scene17-activity rounded-panel border border-sky-200/70 bg-white/95 shadow-card"><header>${icon("signal")}<div><h3>Loop Activity</h3><p>Real-time updates</p></div></header>${[
      ["people", "218 Builders", "joined Stage to Spotlight", "2m ago"],
      ["play", "New Loop Activity", "added to Beat Builders", "5m ago"],
      ["trophy", "Challenge Completed", "in Move the Culture", "12m ago"],
      ["user", "New Builder", "joined Bema Hub", "18m ago"],
    ]
      .map(
        ([itemIcon, title, copy, time]) =>
          `<article>${icon(itemIcon)}<div><strong>${title}</strong><span>${copy}</span><small>${time}</small></div></article>`,
      )
      .join(
        "",
      )}<footer><strong>Ready to participate?</strong><span>Find a campaign that moves you.</span><button>Explore Campaigns</button></footer></aside></div>`;
    return renderLayeredUnderlay("17", config, body);
  },
  renderForeground() {
    return renderLayeredForeground("17", config);
  },
};
