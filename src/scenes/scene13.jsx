// Scene 13 owns these design primitives so it can be edited independently.
const ICONS = {
  people:
  ...
  thumb:
    '<svg viewBox="0 0 24 24"><path d="M8 10 12 3c2 0 3 1.2 2.5 3L14 9h5c1.7 0 2.6 1.2 2.2 2.8l-1.5 6c-.3 1.3-1.2 2.2-2.7 2.2H8V10Zm0 0H3v10h5"/></svg>',
  crown:
    '<svg viewBox="0 0 24 24"><path d="m3 7 5 4 4-7 4 7 5-4-2 11H5L3 7ZM5 21h14"/></svg>',
  trophy:
  ...
  handshake:
    '<svg viewBox="0 0 24 24"><path d="m3 8 4-3 4 2 2-1 4 2 4 4-6 6-3-2-2 1-7-7V8ZM8 13l3-3 5 5M10 17l-2 2M13 18l-1 2"/></svg>',
};
...
function renderBrand({ enrollment = false } = {}) {
  return `
    <div class="broadcast-brand absolute left-[62px] top-[42px] z-20 flex items-center gap-3 border-0 bg-transparent font-sans text-bema-navy shadow-none" style="background:transparent!important;border:0!important;box-shadow:none!important;backdrop-filter:none!important">
      <img class="h-14 w-auto bg-transparent" style="background:transparent!important" src="/logo.png" alt="" />
      <span class="brand-wordmark bg-transparent" style="background:transparent!important;border:0!important;box-shadow:none!important" data-asset-wrapper>
        <img class="broadcast-wordmark h-9 w-auto bg-transparent" style="background:transparent!important" src="/full-logo.png" alt="BemaHub" data-asset-image />
...
function renderLayeredUnderlay(id, config, body) {
  ...
  return sceneMarkup(`
    <section class="scene layered-scene reference-built-scene reference-scene-${id} layout-${config.layout} absolute inset-0 overflow-hidden font-sans text-bema-navy ${config.compact ? "is-compact" : ""} ${config.darkPanel ? "has-dark-panel" : ""}">
      ...
      <header class="layered-scene-heading absolute z-10 bg-white p-4 rounded-3xl text-center">
        ...
        <p class="font-semibold text-black">${config.subtitle}</p>
        ${dottedHeading ? "" : '<span class="layered-heading-rule">♥</span>'}
      </header>
...
const config = {
  title: "COMMUNITY BUILDER TIERS",
  subtitle: "The horizontal spread of Loop Activity.",
  layout: "flow",
  compact: true,
  items: [
    card(
    ...
    card(
      "Partner",
      "Meaningful reach and deeper alignment.",
      "handshake",
      "04",
      "blue",
      "tier-partner",
    ...
    card(
      "Partner Elite",
      "Highest Community Builder Tier.",
      "crown",
      "05",
      "amber",
      "tier-elite",
  ...
  callout:
    "Community Builder Tiers are earned through Loop Activity and qualifying LoopLocks. They are not purchased.",
  footer: [
    ["people", "COMMUNITY BUILDER TIERS"],
    ["thumb", "Builder"],
    ["chart", "Loop Activity"],
    ["lock", "Qualifying LoopLocks"],
  ],
};
export const scene13 = {
  presenterZone: "left",
  renderUnderlay() {
    return renderLayeredUnderlay("13", config, renderTierTrack(config));
  },
  renderForeground() {
    return renderLayeredForeground("13", config);
  },
};
