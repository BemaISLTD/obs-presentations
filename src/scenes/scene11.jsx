// Scene 11 owns these design primitives so it can be edited independently.
const ICONS = {
  people:
...
function renderLiveBadge() {
  return '<div class="broadcast-live absolute right-[52px] top-[34px] z-20 inline-flex items-center gap-2 rounded-xl border border-red-500 bg-red-600 px-5 py-3 text-2xl font-black tracking-[.08em] text-white shadow-lg"><span class="size-3 animate-pulse rounded-full bg-white shadow-[0_0_12px_rgba(255,255,255,.75)]"></span>LIVE</div>';
}
...
function renderLayeredUnderlay(id, config, body) {
  const dottedHeading = ["09", "11", "12", "26", "27"].includes(id);
  return sceneMarkup(`
...
function renderFlow(id, config) {
  return `<div class="reference-flow reference-flow-${id}"><div class="reference-flow-line"></div>${config.items.map((item, index) => `<article class="reference-flow-step tone-${item.tone} ${cardSurface} ${toneClasses(item.tone)} relative grid place-items-center p-4 text-center" data-control-cue="${item.cue}" style="--item-index:${index}"><strong class="reference-flow-index">${item.value}</strong><div class="reference-flow-icon">${icon(item.icon)}</div><h3 class="font-semibold text-3xl mt-4">${item.title}</h3><p class="font-semibold text-lg w-60">${item.copy}</p>${index < config.items.length - 1 ? '<span class="reference-flow-arrow">→</span>' : ""}</article>`).join("")}</div>`;
}
...
const config = {
  ...
  items: [
    ...
    card(
      "Loop Activity",
      "Where your movement and impact are tracked.",
      "chart",
      "04",
      "purple",
      "term-activity",
  ...
  callout:
    "Qualifying LoopLocks may contribute to Recognized Impact according to campaign rules.",
  footer: [
    ["signal", "EchoLoop Explained"],
    ["link", "LoopLink"],
    ["qr", "LoopCode"],
    ["lock", "LoopLock"],
    ["chart", "Loop Activity"],
...
export const scene11 = {
  presenterZone: "left",
  renderUnderlay() {
    const body = `<div class="scene11-echoloop-panel h-full">${renderFlow("11", config)}<div class="scene11-notes"><p class="font-semibold text-lg"><span>i</span>Sharing alone does not create Recognized Impact.</p><p class="font-semibold text-lg"><span>✓</span>Qualifying LoopLocks may contribute to Recognized Impact according to campaign rules.</p></div></div>`;
    return renderLayeredUnderlay("11", config, body);
  },
  renderForeground() {
    return renderLayeredForeground("11", config);
  },
};
