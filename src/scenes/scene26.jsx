...
// Scene 26 owns these design primitives so it can be edited independently.
const ICONS = {
...
const config = {
    title: 'EVENTS PAGE WALKTHROUGH', subtitle: 'Discover live moments, gatherings, and event registration points.', layout: 'dashboard',
    items: [
...
export const scene26 = {
  presenterZone: 'left',
  renderUnderlay() {
    const events = [['MAY', '24', 'Creators Connect Live', 'Building Together in Real Time', 'Join Live'], ['MAY', '28', 'LoopLink Masterclass', 'Share Smarter. Grow Faster.', 'Register'], ['JUN', '02', 'Community Impact Summit', 'Ideas. Action. Impact.', 'Register']]
    const body = `<section class="h-full overflow-hidden rounded-[28px] border border-sky-200 bg-white/95 shadow-2xl"><nav class="flex h-14 items-center gap-8 border-b px-7 text-sm font-bold"><b class="mr-auto text-xl text-blue-700">bema<span class="text-slate-900">Hub</span></b><span>⌂ Home</span><span>♧ Campaigns</span><span class="border-b-2 border-blue-600 py-4 text-blue-600">▣ Events</span><span>♙ Community</span><b class="grid size-8 place-items-center rounded-full bg-violet-600 text-white">JR</b></nav><div class="grid h-[calc(100%-56px)] grid-cols-[1fr_270px] gap-5 p-6"><main><header class="flex items-start justify-between"><div><h3 class="text-3xl font-black">Events</h3><p class="font-semibold text-slate-600">Find live moments, join gatherings,<br/>and register for opportunities.</p></div><div class="flex gap-3"><label class="rounded-lg border px-5 py-3 text-slate-500">Search events...　⌕</label><button class="rounded-lg bg-blue-600 px-5 font-black text-white">+ Create Event</button></div></header><div class="mt-4 flex gap-8 border-b pb-3 text-sm font-bold"><b class="text-blue-600">All Events</b><span>Live Now</span><span>Upcoming</span><span>Past Events</span></div><div class="mt-4 grid gap-3">${events.map(([month, day, title, copy, action], index) => `<article class="grid grid-cols-[74px_1fr_auto] items-center gap-4 rounded-xl border border-slate-200 p-3 shadow-sm"><time class="grid place-items-center rounded-xl ${index === 1 ? 'bg-cyan-50 text-cyan-600' : 'bg-blue-50 text-blue-600'}"><small>${month}</small><b class="text-2xl">${day}</b></time><div><h4 class="text-base font-black">${title}</h4><p class="text-sm font-semibold text-slate-500">${copy}</p><small>◷ 10:00 AM　⌖ Bema Hub</small></div><button class="rounded-lg border-2 border-violet-500 px-5 py-2 font-black text-violet-600">${action}</button></article>`).join('')}</div></main><aside class="rounded-2xl border border-slate-200 p-5"><h3 class="text-lg font-black">Upcoming Moments</h3>${['Weekly Creator Check-In', 'Builders Office Hours', 'Innovation Spotlight', 'Community Town Hall'].map((item, index) => `<div class="border-b py-5"><strong class="block">${item}</strong><small class="text-slate-500">${index + 3} Jun · ${12 + index}:00 PM</small></div>`).join('')}</aside></div></section>`
    return renderTailwindUnderlay({ title: 'EVENTS PAGE<br/>WALKTHROUGH', subtitle: config.subtitle, body })
  },
  renderForeground() { return renderTailwindForeground(['▣ Events Page', '◷ Upcoming Events', '⌁ Join Live Moments']) },
}
