  ...
  return `
    <div class="broadcast-brand absolute left-[62px] top-[42px] z-20 flex items-center gap-3 font-sans text-bema-navy">
      <span class="brand-wordmark" data-asset-wrapper>
        <img class="broadcast-wordmark h-12 w-auto" src="/full-logo.png" alt="BemaHub" data-asset-image />
...
// The studio room the approved plate used to supply as a photograph. It is
// drawn now because the presenter is a live camera layer above this scene, and
// the plate had a host baked into it — a picture of a presenter behind the
// real one. Geometry follows the reference: wall-mounted screen upper right,
// console beneath it, and the left third left clear for the camera.
function renderStudioRoom() {
  return `
    <div class="scene03-room" aria-hidden="true">
      <div class="scene03-room-wall"></div>
      <div class="scene03-room-sweep"></div>
      <div class="scene03-room-console"></div>
      <div class="scene03-tv">
        <div class="scene03-tv-screen"></div>
        <div class="scene03-tv-stand"></div>
      </div>
    </div>
  `;
}
...
export const scene03 = {
  ...
  renderUnderlay() {
    return sceneMarkup(
      `<section class="scene proof-scene scene03 proof-enter absolute inset-0 overflow-hidden font-sans text-bema-navy">${renderStudioRoom()}${renderBrand()}${renderLiveBadge()}<div class="scene03-presenter-space" aria-label="Presenter camera placement"></div><article class="studio-display mr-24 -mt-16 "><h2 class="font-semibold"><span class="text-red-500">bema</span> Hub</h2><h4 class="text-2xl">Better Benefits.<br>Stronger Together.</h4><div class="studio-values"><div>${icon("music")}<span>Creative<br>Value</span></div><div>${icon("people")}<span>Builders</span></div><div>${icon("signal")}<span>Loop<br>Activity</span></div><div>${icon("heart")}<span>Recognized<br>Impact</span></div></div></article></section>`,
    );
  },
  renderForeground(context) {
    ...
    return sceneMarkup(
      `<section class="scene-foreground scene03-foreground"><div class="host-lower-third" data-control-cue="reveal-lower-third"><span>HOST</span><strong data-scene03-presenter-name>${presenterName}</strong><p>Community Manager, Bema Hub</p></div>${renderForegroundBar(
        [
